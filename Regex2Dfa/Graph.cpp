...
void CBCompiler::Graph::GenerateRelation(int from, int to, const DFA::Token &token) {
...
void CBCompiler::Graph::DrawDot(const std::string &outf) {
    ...
    out << "node_1 [label=start shape=doublecircle color=blue];" << std::endl;
    for (uint i = 2; i <= node_id; ++i) {
        if (acc_states.count(i)) {
            out << string_format("node_%d [label=%s shape=%s color=red];", i, "Accept", "doublecircle") << std::endl;
        } else {
