...
class Function : public Value {
    ...
    std::set<Function *> callee_func;
    // functions calling this function
    std::set<Function *> caller_func;
    bool builtin;
    bool load_global;
...
class Instruction : Value {
    ...
    Instruction(Tag tag, Instruction *insertBefore) : Value(tag), bb(insertBefore->bb) {
        bb->insts.insertBefore(this, insertBefore);
    }

    // insert this inst at the end of `insertAtEnd`
    Instruction(Tag tag, BasicBlock *insertAtEnd) : Value(tag), bb(insertAtEnd) { bb->insts.insertAtEnd(this); }

    // 只初始化tag，没有加入到链表中，调用者手动加入
    Instruction(Tag tag) : Value(tag) {}
...
class BinaryInst : Instruction {
    ...
    BinaryInst(Tag tag, Value *lhs, Value *rhs, Instruction *insertBefore)
...
class AccessInst : Instruction {
    Decl *lhs_sym;
    Use arr;
    Use index;

    AccessInst(Instruction::Tag tag, Decl *lhs_sym, Value *arr, Value *index, BasicBlock *insertAtEnd)
            : Inst(tag, insertAtEnd), lhs_sym(lhs_sym), arr(arr, this), index(index, this) {}
};

class GetElementPtrInst : AccessInst {
    int multiplier;
...
class CallInst : Instruction {
    ...
    CallInst(Function *func, BasicBlock *insertAtEnd) : Inst(Tag::Call, insertAtEnd), func(func) {}
};
...
class PhiInst : Instruction {
    ...
    explicit PhiInst(Instruction *insertBefore) : Inst(Tag::Phi, insertBefore) {
        u32 n = incoming_bbs().size();
        incoming_values.reserve(n);
        for (u32 i = 0; i < n; ++i) {
            incoming_values.emplace_back(nullptr, this);
